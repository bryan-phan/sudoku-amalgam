...
1. **Find the board.** Detect the puzzle's outer contour and correct the perspective distortion so a photo taken at an angle becomes a flat, square grid.
2. **Segment the cells.** Split the warped grid into 81 individual cell crops.
3. **Read the givens.** Extract a digit mask from each cell, match it against generated digit templates, and reject low-confidence matches before committing anything to the grid, so a misread is left blank rather than guessed wrong.
4. **Solve.** Apply ten constraint-solving techniques to prune the search space, then finish with backtracking search only where logic alone cannot.
